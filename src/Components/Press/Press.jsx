import React, { Component } from 'react';
import { Image } from 'react-bootstrap';

import './Press.css';

const press = props => (
  <div className="press-container">

    <div className="press-info compact">
      <h3>Press</h3>

      <h4>PRESSRELEASE ”BIOTEATER VÄNNERNA I KUNGASKOGEN”</h4>
      <p>Snart har 100 000 förskolebarn gått på bioteater för att se succèföreställning om vänskap.</p>
      <p>Vi i malmö har snart lyckats samla 100 000 förskolebarn till bioteaterföreställningar i hela sverige.</p>
      <p>Bioteatern ”vännerna i kungaskogen” har visats på över 50 orter i sverige från trelleborg i söder till boden i norr.</p>
      <p>De flesta föreställningar är för det mesta utsålda och handlar om vänskap, respekt för andra och mot utanförskap och mobbning.</p>
      <p>Sveriges första digitala barnteater (teater på bioduk; bioteater) visas nu för sjätte året i rad runtom i sverige.</p>
      <p>Intresset från pedagoger och förskolor är rekordstort. Förskolebarnen kommer för att se när kompisarna ekorren snick och igelkotten snack från kungaskogen ska bygga en kompiskoja. Budskapet i sagan om vännerna i kungaskogen är viktigt; ”att vara schysst och en bra kompis”.</p>
      <p>Flera av barnviseklassikerna från kungaskogen framförs och sjungs också på bioduken. I en av sångerna bjuds upp till dans i biosalongen.</p>
      <p>De allra bästa musikerna och barnmusikalartisterna medverkar i sångproduktionerna. Föreställningen inleds av dramatenskådespelaren börje ahlstedt. På spotify (sökord kungaskogen) finns</p>
      <p>några av de sånger som framförs i bok- och musiksagan, snick och snack vännerna i kungaskogen bygger en koja som över 9000 förskolor i sverige från ystad till haparanda använder.</p>
      <p>Snick och snack är viktiga förebilder för barnen. Nu kan viktiga värdegrunder tillgängligöras i stora delar av sverige genom modern digital teknik när teaterversionen av verket ”vännerna i kungaskogen” också kan visas på bioduk.</p>
      <p>Bioteatern vännerna i kungaskogen grundar sig på bok och musikverket ”vännerna i kungaskogen bygger en koja” som är producerad i malmö och som gjort succé på förskolor i hela sverige.</p>

      <h4>Medverkande i bioföreställningen</h4>
      <p>Berättare inledning: Börje Ahlstedt</p>
      <p>Musikalartister: My Johansson, Louisa Lyne och Tobias Kralmark. Regi: Magnus Ödéhn.</p>
      <p>Sång på cd:n: Ville och Vännerorkestern.</p>
      <p>Klippning och redigering: Good World AB Malmö</p>
      <p>Filmning: Solid Entertainment AB Malmö</p>

      <h4>Upphovspersoner till verket vännerna i kungaskogen</h4>
      <p>Text och manus: Lisa Dos Santos</p>
      <p>Musik: Lisa Dos Santos, Joc Lind, Vilhelm Ersmark</p>
      <p>Förlag: ILKA Förlagsprodukter AB/Wasa Music AB Malmö</p>
      <p>Spotify: använd sökord kungaskogen</p>

      <h4>Kontaktperson</h4>
      <p>040-120012 Ulf</p>
      <p>idea@ilka.se</p>

      <a href="" >Trailer Bioteater Kungaskogen</a>
      <a href="" >Lyssna när 800 glada barn och vuxna blir vänner med djurkompisarna i kungaskogen</a>

      <h4>Åklagare skrev förskolans mest använda sagobok</h4>

      <p>Under pseudonymen Lisa Grane, numera Lisa dos Santos, skrev Lisa manus och musiktexter till vännerna i Kungaskogen som blivit förskolans mest använda saga under 2000-talet.</p>
      <p>När Lisa inte skriver barnböcker arbetar hon som åklagare i stockholm.</p>
      <p>– Jag tror att det är väldigt viktigt att i tidig barndom lära sig att ha respekt för andra och känna och förstå empati. Det är så grundläggande hur man ska vara mot varandra. Förstår man hur man
      ska behandla människor i ens omgivning har man behållning av det livet ut. Jag ville göra en saga som tog upp det här på ett lättsamt sätt. Samtidigt ville jag att det skulle nå ut till barnen på
      flera nivåer så att de själva kunde vara med och delta i berättelsen. Därför blev också musiken ett viktigt inslag i sagan, säger Lisa.</p>
      <p>Lisa arbetar med nya sagor och texter till Kungaskogen på sin fritid.</p>
      <p>– Fler böcker och sånger planeras tillsammans med förlaget ILKA, fortsätter Lisa.</p>
      <p>Lisa bor som så många andra kreativa människor på Södermalm i Stockholm. På promenader i de gamla söderkvarteren och i vardagliga möten med människor hämtar Lisa nya idéer till manus.</p>

      <p>– Men ibland måste man åka iväg alldeles ensam någonstans för att kunna slutföra texter. Mitt sista manus till Kungaskogen färdigställde jag i en liten stuga på Fårö på Gotland. Här fanns lugnet och när jag tittade ut genom fönstret och såg Fårölandskapet såg jag också vad Snick och Snack och alla vännerna gjorde i Kungaskogen.</p>

      <p>– Jag är glad och stolt över att Kungaskogen har blivit ett så populärt och användarvänligt material för förskolan. Jag har också genom förlaget dagligen fått ta del av all den positiva respons
      som pedagoger och barn förmedlat. Särskilt glad är jag att så många berättat att barn som läst och arbetat med sagan faktiskt blivit lite snällare mot varandra!</p>
      <p>Lisa är uppväxt i en stor familj med fem syskon, där det alltid funnits liv och lek runt omkring.</p>
      <p>– Eftersom jag alltid gillat att skriva såg jag till att få sitta för mig själv ibland och fantisera ihop alla möjliga berättelser, dikter och sagor. Dock hade jag svårt att bestämma mig för vilket område
      som var min specialitet när det gällde att skriva, så jag påbörjade allt ifrån hästhistorier till krigsskildringar. Tills slut blev det barnböcker.</p>


    </div>

    <h3>Bilder</h3>
    <div className="press-image">
      <Image src="images/Ovrigt/snick_och_snack.jpg" />
      <Image src="images/Ovrigt/boken_kojan.jpg" />
      <Image src="images/Ovrigt/lisa.jpg" />
    </div>
    <p>Kontakta gärna förlaget så får ni en kod för mer material om böckerna om Snack & Snack och Vännerna i Kungaskogen.</p>
  </div>

);

export default press;
